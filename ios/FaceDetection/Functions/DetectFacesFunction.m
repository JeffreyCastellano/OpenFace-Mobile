...
FREObject fd_detectFaces( FREContext context, void* functionData, uint32_t argc, FREObject argv[] ) {
    ...
    if( FREAcquireBitmapData2( imageObject, &bitmapData ) == FRE_OK ) {
        BOOL detectSaveImage = [MPFREObjectUtils getBOOL:argv[2]];
        BOOL detectAUs = [MPFREObjectUtils getBOOL: argv[3]];
        BOOL detectGaze = [MPFREObjectUtils getBOOL: argv[4]];
        BOOL detectPose = [MPFREObjectUtils getBOOL: argv[5]];
        
        [[FaceDetectionHelper sharedInstance] detectFaces:bitmapData detectSaveImage:detectSaveImage detectAUs:detectAUs detectGaze:detectGaze detectPose:detectPose callbackId:callbackId];
